...
if (isset($_POST['reset'])) {
    ...
    if ($stmt = $con->prepare("UPDATE student SET studentPW = ? WHERE studentID = ?")) {
        $stmt->bind_param('ss', $newPassword, $studentID); // Bind the parameters
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>
                ...
                window.location.replace('forgot_pw.S.html');
            </script>";
        }
        $stmt->close();
    } else {
